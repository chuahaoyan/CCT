...
class Attention(Module):
    ...
    def __init__(self, dim, num_heads=8, attention_dropout=0.1, projection_dropout=0.1):
        ...
        self.register_parameter('k_ext', None)
    ...
    def forward(self, x):
        ...
        if self.attn_bias is None:
            self.attn_bias = Parameter(torch.randn(1,N,device='cuda:0'))
        attn_bias = self.attn_bias

        if self.k_ext is None:
            self.k_ext = Parameter(torch.randn(N, C//self.num_heads, device='cuda:0'))
        k = self.k_ext
...
class Mlp(Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=F.gelu, dropout=0.1):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        self.fc1 = Linear(in_features, hidden_features)
        self.act = act_layer() #GELU layer
...
class TransformerEncoderLayer(Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        self.conv_one = Conv1d(in_channels=d_model,out_channels=d_model,kernel_size=1)
        self.norm = LayerNorm(d_model)

    def forward(self, src: torch.Tensor, *args, **kwargs) -> torch.Tensor:
        src = src + self.drop_path(self.norm(self.conv_one(self.self_attn(self.pre_norm(src)))))
        src = self.norm1(src)
        src2 = self.mlp(src)
